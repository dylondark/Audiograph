    ...
    public partial class frmConsole
    {
        ...
        public void CommandInterpreter(string command)
        {
            ...
            switch (commandarray[0] ?? "")
            {
                ...
                case "removeuser":
                    {
                        My.MySettingsProperty.Settings.User = string.Empty;
                ...
                case "removesk":
                    {
                        My.MySettingsProperty.Settings.SessionKey = string.Empty;
                        My.MyProject.Forms.frmMain.AuthenticatedUI(false);
                ...
                case "ram":
                    {
                        ...
                        DisplayOut((x.WorkingSet64 / 1024d / 1024d).ToString("N2") + "MB currently in use with " + x.Threads.Count.ToString() + " threads by the Audiograph process.");
                        break;
                    }
                case "apiinfo":
                    {
                        ...
                        if (authorized == true)
                        {
                            DisplayOut("Key".PadRight(8) + Secrets.APIkey + Constants.vbCrLf + "Secret".PadRight(8) + Secrets.APIsecret);
                        }
                        else
                        {
        ...
        private void CmdLfm(string[] commands)
        {
            ...
            if (commands.Count() >= 3) // has required parameters
            {
                ...
                if (commands[1] == "get")
                {
                    ...
                    string returnXML = Utilities.CallAPI(newcommands[2], newcommands[3], newcommands[4], newcommands[5], newcommands[6]);
                ...
                else if (commands[1] == "post")
                {
                    ...
                    if (authorized == true)
                    {
                        ...
                        string returnXML = Utilities.CallAPIAuth(newcommands[2], newcommands[3], newcommands[4], newcommands[5]);
        ...
        private void CmdVar(string[] commands)
        {
            ...
            if (commands.Count() >= 2)
            {
                ...
                switch (commands[1] ?? "")
                {
                    case "apihistory":
                        {
                            output.AppendLine("Displaying variable 'apihistory' (frmMain) as List of String with " + Utilities.apihistory.Count.ToString() + " elements.");
                            output.Append(string.Empty);
                            // display values
                            for (uint count = 0U, loopTo = (uint)(Utilities.apihistory.Count - 1); count <= loopTo; count++)
                                output.Append(Constants.vbCrLf + "apihistory (" + count + ") = " + Utilities.apihistory[(int)count][0] + ", " + Utilities.apihistory[(int)count][1] + ", " + Utilities.apihistory[(int)count][2] + ", " + Utilities.apihistory[(int)count][3] + ", " + Utilities.apihistory[(int)count][4] + ", " + Utilities.apihistory[(int)count][5]);
                            DisplayOut(output.ToString());
                            break;
                        }
                    case "tracklookup":
                        {
                            output.AppendLine("Displaying variable 'tracklookup' (frmMain) as Array of String with " + Utilities.tracklookup.Count().ToString() + " elements.");
                            output.Append(string.Empty);
                            // display values
                            for (uint count = 0U, loopTo1 = (uint)(Utilities.tracklookup.Count() - 1); count <= loopTo1; count++)
                                output.Append(Constants.vbCrLf + "tracklookup(" + count + ") = " + Utilities.tracklookup[(int)count]);
                            DisplayOut(output.ToString());
                            break;
                        }
                    case "cmdlist":
                    ...
                    case "cmdlistcount":
                        {
                            DisplayOut("Displaying variable 'cmdlistcount' (frmConsole) as UShort." + Constants.vbCrLf + cmdlistcount.ToString());
                            break;
                        }
                    case "progress":
                        {
                            DisplayOut("Displaying variable 'progress' (frmMain) as UShort." + Constants.vbCrLf + Utilities.progress.ToString());
                            break;
                        }
                    case "progressmultiplier":
                        {
                            DisplayOut("Displaying variable 'progressmultiplier' (frmMain) as Byte." + Constants.vbCrLf + Utilities.progressmultiplier.ToString());
                            break;
                        }
                    case "stoploadexecution":
                        {
                            DisplayOut("Displaying variable 'stoploadexecution' (frmMain) as Boolean." + Constants.vbCrLf + Utilities.stoploadexecution.ToString());
                            break;
                        }
                    case "userlookup":
                        {
                            DisplayOut("Displaying variable 'userlookup' (frmMain) as String." + Constants.vbCrLf + Utilities.userlookup);
                            break;
                        }
                    case "settings.user":
                        {
                            DisplayOut("Displaying settings variable 'user' as String." + Constants.vbCrLf + My.MySettingsProperty.Settings.User);
                            break;
                        }
                    case "settings.sessionkey":
                        {
                            ...
                            if (authorized == true)
                            {
                                DisplayOut("Displaying settings variable 'sessionkey' as String." + Constants.vbCrLf + My.MySettingsProperty.Settings.SessionKey);
                            }
                            else
                            {
        ...
        private void CmdCreatesig(string[] commands)
        {
            ...
            if (commands.Count() >= 2)
            {
                ...
                if (authorized == true)
                {
                    ...
                    DisplayOut(Utilities.CreateSignature(newcommands[1], newcommands[2], newcommands[3], newcommands[4]));
                }
                else
                {
                                ...
                                case "authentication":
                                    {
                                        ...
                                        My.MyProject.Forms.frmAuthentication.Activate();
                                        DisplayOut(string.Empty);
                                        break;
                                    }
                                case "apihistory":
                                ...
                                case "scrobbleindextrackadvanced":
                                    {
                                        ...
                                        My.MyProject.Forms.frmScrobbleIndexTrackAdvanced.Activate();
                                        DisplayOut(string.Empty);
                                        break;
                                    }
                                case "scrobblesearch":
                    ...
                    case "close":
                        {
                            switch (commands[2].ToLower() ?? "")
                            {
                                ...
                                case "authentication":
                                    {
                                        My.MyProject.Forms.frmAuthentication.Close();
                                        DisplayOut(string.Empty);
                                        break;
                                    }
                                case "apihistory":
                                    {
                                        My.MyProject.Forms.frmAPIHistory.Close();
                                        DisplayOut(string.Empty);
                                        break;
                                    }
                                case "trackadvanced":
                                    {
                                        My.MyProject.Forms.frmTrackAdvanced.Close();
                                        DisplayOut(string.Empty);
                                        break;
                                    }
                                case "artistadvanced":
                                    {
                                        My.MyProject.Forms.frmArtistAdvanced.Close();
                                        DisplayOut(string.Empty);
                                        break;
                                    }
                                case "albumadvanced":
                                    {
                                        My.MyProject.Forms.frmAlbumAdvanced.Close();
                                        DisplayOut(string.Empty);
                                        break;
                                    }
                                case "adddqueue":
                                    {
                                        My.MyProject.Forms.frmAddToQueue.Close();
                                        DisplayOut(string.Empty);
                                        break;
                                    }
                                case "console":
                                ...
                                case "scrobblehistory":
                                    {
                                        My.MyProject.Forms.frmScrobbleHistory.Close();
                                        DisplayOut(string.Empty);
                                        break;
                                    }
                                case "backuptool":
                                    {
                                        My.MyProject.Forms.frmBackupTool.Close();
                                        DisplayOut(string.Empty);
                                        break;
                                    }
                                case "scrobbleindexeditor":
                                    {
                                        My.MyProject.Forms.frmScrobbleIndexEditor.Close();
                                        DisplayOut(string.Empty);
                                        break;
                                    }
                                case "scrobbleindexaddrow":
                                ...
                                case "scrobblesearch":
                                    {
                                        My.MyProject.Forms.frmScrobbleSearch.Close();
                                        DisplayOut(string.Empty);
                                        break;
                                    }
                                ...
                                case "main":
                                    {
                                        ...
                                        output.AppendLine(string.Empty);
                                        output.AppendLine("Open".PadRight(12) + My.MyProject.Forms.frmMain.Visible.ToString());
                                        output.AppendLine("Size".PadRight(12) + My.MyProject.Forms.frmMain.Width.ToString() + ", " + Height);
                                        output.AppendLine("Position".PadRight(12) + My.MyProject.Forms.frmMain.Left.ToString() + ", " + My.MyProject.Forms.frmMain.Top.ToString());
                                        output.Append("WindowState".PadRight(12) + My.MyProject.Forms.frmMain.WindowState.ToString());
                                        DisplayOut(output.ToString());
                                        break;
                                    }
                                case "about":
                                    {
                                        ...
                                        output.AppendLine(string.Empty);
                                        output.AppendLine("Open".PadRight(12) + My.MyProject.Forms.frmAbout.Visible.ToString());
                                        output.AppendLine("Size".PadRight(12) + My.MyProject.Forms.frmAbout.Width.ToString() + ", " + My.MyProject.Forms.frmAbout.Height);
                                        output.AppendLine("Position".PadRight(12) + My.MyProject.Forms.frmAbout.Left.ToString() + ", " + My.MyProject.Forms.frmAbout.Top.ToString());
                                        output.Append("WindowState".PadRight(12) + My.MyProject.Forms.frmAbout.WindowState.ToString());
                                        DisplayOut(output.ToString());
                                        break;
                                    }
                                case "authentication":
                                    {
                                        ...
                                        output.AppendLine(string.Empty);
                                        output.AppendLine("Open".PadRight(12) + My.MyProject.Forms.frmAuthentication.Visible.ToString());
                                        output.AppendLine("Size".PadRight(12) + My.MyProject.Forms.frmAuthentication.Width.ToString() + ", " + My.MyProject.Forms.frmAuthentication.Height);
                                        output.AppendLine("Position".PadRight(12) + My.MyProject.Forms.frmAuthentication.Left.ToString() + ", " + My.MyProject.Forms.frmAuthentication.Top.ToString());
                                        output.Append("WindowState".PadRight(12) + My.MyProject.Forms.frmAuthentication.WindowState.ToString());
                                        DisplayOut(output.ToString());
                                        break;
                                    }
                                case "apihistory":
                                    {
                                        ...
                                        output.AppendLine(string.Empty);
                                        output.AppendLine("Open".PadRight(12) + My.MyProject.Forms.frmAPIHistory.Visible.ToString());
                                        output.AppendLine("Size".PadRight(12) + My.MyProject.Forms.frmAPIHistory.Width.ToString() + ", " + My.MyProject.Forms.frmAPIHistory.Height);
                                        output.AppendLine("Position".PadRight(12) + My.MyProject.Forms.frmAPIHistory.Left.ToString() + ", " + My.MyProject.Forms.frmAPIHistory.Top.ToString());
                                        output.Append("WindowState".PadRight(12) + My.MyProject.Forms.frmAPIHistory.WindowState.ToString());
                                        DisplayOut(output.ToString());
                                        break;
                                    }
                                case "trackadvanced":
                                    {
                                        ...
                                        output.AppendLine(string.Empty);
                                        output.AppendLine("Open".PadRight(12) + My.MyProject.Forms.frmTrackAdvanced.Visible.ToString());
                                        output.AppendLine("Size".PadRight(12) + My.MyProject.Forms.frmTrackAdvanced.Width.ToString() + ", " + My.MyProject.Forms.frmTrackAdvanced.Height);
                                        output.AppendLine("Position".PadRight(12) + My.MyProject.Forms.frmTrackAdvanced.Left.ToString() + ", " + My.MyProject.Forms.frmTrackAdvanced.Top.ToString());
                                        output.Append("WindowState".PadRight(12) + My.MyProject.Forms.frmTrackAdvanced.WindowState.ToString());
                                        DisplayOut(output.ToString());
                                        break;
                                    }
                                case "artistadvanced":
                                    {
                                        ...
                                        output.AppendLine(string.Empty);
                                        output.AppendLine("Open".PadRight(12) + My.MyProject.Forms.frmArtistAdvanced.Visible.ToString());
                                        output.AppendLine("Size".PadRight(12) + My.MyProject.Forms.frmArtistAdvanced.Width.ToString() + ", " + My.MyProject.Forms.frmArtistAdvanced.Height);
                                        output.AppendLine("Position".PadRight(12) + My.MyProject.Forms.frmArtistAdvanced.Left.ToString() + ", " + My.MyProject.Forms.frmArtistAdvanced.Top.ToString());
                                        output.AppendLine("WindowState".PadRight(12) + My.MyProject.Forms.frmArtistAdvanced.WindowState.ToString());
                                        DisplayOut(output.ToString());
                                        break;
                                    }
                                case "albumadvanced":
                                    {
                                        ...
                                        output.AppendLine(string.Empty);
                                        output.AppendLine("Open".PadRight(12) + My.MyProject.Forms.frmAlbumAdvanced.Visible.ToString());
                                        output.AppendLine("Size".PadRight(12) + My.MyProject.Forms.frmAlbumAdvanced.Width.ToString() + ", " + My.MyProject.Forms.frmAlbumAdvanced.Height);
                                        output.AppendLine("Position".PadRight(12) + My.MyProject.Forms.frmAlbumAdvanced.Left.ToString() + ", " + My.MyProject.Forms.frmAlbumAdvanced.Top.ToString());
                                        output.AppendLine("WindowState".PadRight(12) + My.MyProject.Forms.frmAlbumAdvanced.WindowState.ToString());
                                        DisplayOut(output.ToString());
                                        break;
                                    }
                                case "addqueue":
                                    {
                                        ...
                                        output.AppendLine(string.Empty);
                                        output.AppendLine("Open".PadRight(12) + My.MyProject.Forms.frmAddToQueue.Visible.ToString());
                                        output.AppendLine("Size".PadRight(12) + My.MyProject.Forms.frmAddToQueue.Width.ToString() + ", " + My.MyProject.Forms.frmAddToQueue.Height);
                                        output.AppendLine("Position".PadRight(12) + My.MyProject.Forms.frmAddToQueue.Left.ToString() + ", " + My.MyProject.Forms.frmAddToQueue.Top.ToString());
                                        output.AppendLine("WindowState".PadRight(12) + My.MyProject.Forms.frmAddToQueue.WindowState.ToString());
                                        DisplayOut(output.ToString());
                                        break;
                                    }
                                case "console":
                                ...
                                case "scrobblehistory":
                                    {
                                        ...
                                        output.AppendLine(string.Empty);
                                        output.AppendLine("Open".PadRight(12) + My.MyProject.Forms.frmScrobbleHistory.Visible.ToString());
                                        output.AppendLine("Size".PadRight(12) + My.MyProject.Forms.frmScrobbleHistory.Width.ToString() + ", " + My.MyProject.Forms.frmScrobbleHistory.Height);
                                        output.AppendLine("Position".PadRight(12) + My.MyProject.Forms.frmScrobbleHistory.Left.ToString() + ", " + My.MyProject.Forms.frmScrobbleHistory.Top.ToString());
                                        output.AppendLine("WindowState".PadRight(12) + My.MyProject.Forms.frmScrobbleHistory.WindowState.ToString());
                                        DisplayOut(output.ToString());
                                        break;
                                    }
                                case "backuptool":
                                    {
                                        ...
                                        output.AppendLine(string.Empty);
                                        output.AppendLine("Open".PadRight(12) + My.MyProject.Forms.frmBackupTool.Visible.ToString());
                                        output.AppendLine("Size".PadRight(12) + My.MyProject.Forms.frmBackupTool.Width.ToString() + ", " + My.MyProject.Forms.frmBackupTool.Height);
                                        output.AppendLine("Position".PadRight(12) + My.MyProject.Forms.frmBackupTool.Left.ToString() + ", " + My.MyProject.Forms.frmBackupTool.Top.ToString());
                                        output.AppendLine("WindowState".PadRight(12) + My.MyProject.Forms.frmBackupTool.WindowState.ToString());
                                        DisplayOut(output.ToString());
                                        break;
                                    }
                                case "scrobbleindexeditor":
                                    {
                                        ...
                                        output.AppendLine(string.Empty);
                                        output.AppendLine("Open".PadRight(12) + My.MyProject.Forms.frmScrobbleIndexEditor.Visible.ToString());
                                        output.AppendLine("Size".PadRight(12) + My.MyProject.Forms.frmScrobbleIndexEditor.Width.ToString() + ", " + My.MyProject.Forms.frmScrobbleIndexEditor.Height);
                                        output.AppendLine("Position".PadRight(12) + My.MyProject.Forms.frmScrobbleIndexEditor.Left.ToString() + ", " + My.MyProject.Forms.frmScrobbleIndexEditor.Top.ToString());
                                        output.AppendLine("WindowState".PadRight(12) + My.MyProject.Forms.frmScrobbleIndexEditor.WindowState.ToString());
                                        DisplayOut(output.ToString());
                                        break;
                                    }
                                case "scrobbleindexaddrow":
                                    {
                                        ...
                                        output.AppendLine(string.Empty);
                                        output.AppendLine("Open".PadRight(12) + My.MyProject.Forms.frmScrobbleIndexAddRow.Visible.ToString());
                                        output.AppendLine("Size".PadRight(12) + My.MyProject.Forms.frmScrobbleIndexAddRow.Width.ToString() + ", " + My.MyProject.Forms.frmScrobbleIndexAddRow.Height);
                                        output.AppendLine("Position".PadRight(12) + My.MyProject.Forms.frmScrobbleIndexAddRow.Left.ToString() + ", " + My.MyProject.Forms.frmScrobbleIndexAddRow.Top.ToString());
                                        output.AppendLine("WindowState".PadRight(12) + My.MyProject.Forms.frmScrobbleIndexAddRow.WindowState.ToString());
                                        DisplayOut(output.ToString());
                                        break;
                                    }
                                case "scrobbleindextrackadvanced":
                                    {
                                        ...
                                        output.AppendLine(string.Empty);
                                        output.AppendLine("Open".PadRight(12) + My.MyProject.Forms.frmScrobbleIndexTrackAdvanced.Visible.ToString());
                                        output.AppendLine("Size".PadRight(12) + My.MyProject.Forms.frmScrobbleIndexTrackAdvanced.Width.ToString() + ", " + My.MyProject.Forms.frmScrobbleIndexTrackAdvanced.Height);
                                        output.AppendLine("Position".PadRight(12) + My.MyProject.Forms.frmScrobbleIndexTrackAdvanced.Left.ToString() + ", " + My.MyProject.Forms.frmScrobbleIndexTrackAdvanced.Top.ToString());
                                        output.AppendLine("WindowState".PadRight(12) + My.MyProject.Forms.frmScrobbleIndexTrackAdvanced.WindowState.ToString());
                                        DisplayOut(output.ToString());
                                        break;
                                    }
                                case "scrobblesearch":
                                    {
                                        ...
                                        output.AppendLine(string.Empty);
                                        output.AppendLine("Open".PadRight(12) + My.MyProject.Forms.frmScrobbleSearch.Visible.ToString());
                                        output.AppendLine("Size".PadRight(12) + My.MyProject.Forms.frmScrobbleSearch.Width.ToString() + ", " + My.MyProject.Forms.frmScrobbleSearch.Height);
                                        output.AppendLine("Position".PadRight(12) + My.MyProject.Forms.frmScrobbleSearch.Left.ToString() + ", " + My.MyProject.Forms.frmScrobbleSearch.Top.ToString());
                                        output.AppendLine("WindowState".PadRight(12) + My.MyProject.Forms.frmScrobbleSearch.WindowState.ToString());
                                        DisplayOut(output.ToString());
                                        break;
                                    }
        ...
        private void CmdSetsk(string[] commands)
        {
            ...
            if (commands.Count() >= 2)
            {
                ...
                if (authorized == true)
                {
                    ...
                    string oldsk = My.MySettingsProperty.Settings.SessionKey;

                    // set session key
                    My.MySettingsProperty.Settings.SessionKey = commands[1];
                    ...
                    {
                        My.MySettingsProperty.Settings.SessionKey = oldsk;
                    ...
                    {
                        // second step
                        string response2 = Utilities.CallAPI("artist.getTags", My.MySettingsProperty.Settings.User, "artist=test");
        ...
        private void CmdResetprog()
        {
            ...
            My.MyProject.Forms.frmMain.UpdateProgress.Value = 100;
            My.MyProject.Forms.frmMain.UpdateProgress.Visible = false;
            DisplayOut(string.Empty);
        }
        ...
        {
            var output = new StringBuilder();
            output.AppendLine("Current Time: " + DateTime.Now.ToString("G") + " (" + Utilities.DateToUnix(DateTime.Now).ToString() + ")");
            output.AppendLine("Current Time (UTC): " + Utilities.UnixToDate(Utilities.GetCurrentUTC()).ToString("G") + " (" + Utilities.GetCurrentUTC().ToString() + ")");
            output.Append("Time Zone Offset: " + (Utilities.timezoneoffset / 3600d).ToString() + ":00 (" + Utilities.timezoneoffset.ToString() + ")");
            DisplayOut(output.ToString());
        }
        ...
        private void CmdViewsk()
        {
            ...
            {
                DisplayOut("Current session key is '" + My.MySettingsProperty.Settings.SessionKey + "' for user '" + My.MySettingsProperty.Settings.User + "'");
            }
            else
            {
        ...
        private void History(object sender, KeyEventArgs e)
        {
            ...
            {
                e.Handled = true;
                if (_History_checkvalue != cmdlist.Count)  // new command check (if checkvalue is different than cmdlist.count then a new command has been entered)
                {

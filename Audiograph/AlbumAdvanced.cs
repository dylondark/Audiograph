...
namespace Audiograph
{
    ...
    public partial class frmAlbumAdvanced
    {
        ...
        private void Search(object sender, EventArgs e)
        {
            ...
            {
                results = Utilities.CallAPI("album.search", "", "album=" + txtSearchAlbum.Text.Trim().Replace(" ", "+"), "artist=" + txtSearchArtist.Text.Trim().Replace(" ", "+"));
            }
            ...
            if (status.Contains("ok") == true)
            {
                ...
                {
                    // parse for data
                    string[] searchnodes = new string[] { "name", "artist" };
                        ...
                        Invoke(new Action(() =>
   {
                        ...
                        responseXML = Utilities.CallAPI("album.getInfo", My.MySettingsProperty.Settings.User, "album=" + album, "artist=" + artist);

                        // parse data
                        string[] albumnodes = new[] { "name", "artist", "listeners" };
       ...
       {
           // album name
                                if (albumnodes[0].Contains("ERROR: ") == false)
                                {
                                    txtAlbum.Text = albumnodes[0];
                                }
           // artist name
                                if (albumnodes[1].Contains("ERROR: ") == false)
                                {
        ...
        {
            // call api
            string info = Utilities.CallAPI("album.getInfo", My.MySettingsProperty.Settings.User, "mbid=" + txtMBID.Text.Trim().ToLower());

            // get info or error
            string[] infonodes = new string[] { "name", "artist", "listeners" };
        ...
        {
            if (!string.IsNullOrEmpty(txtAlbum.Text) && !string.IsNullOrEmpty(txtArtist.Text) && txtArtist.Text != "N/A" && txtArtist.Text != "N/A")    // if there is data in both boxes
            {
                Utilities.GoToAlbum(txtAlbum.Text.Trim(), txtArtist.Text.Trim());
                Close();                                      // close window
            }
            else if (string.IsNullOrEmpty(txtTracks.Text) && !string.IsNullOrEmpty(txtArtist.Text)) // if there is only data in album
            {
            ...
            else if (string.IsNullOrEmpty(txtArtist.Text) && !string.IsNullOrEmpty(txtTracks.Text)) // if there is only data in artist
            {
